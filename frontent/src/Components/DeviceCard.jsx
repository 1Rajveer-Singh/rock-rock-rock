...
const DeviceCard = ({ 
    ...
    try {
      ...
      if (response.ok) {
        ...
        progressInterval.current = setInterval(async () => {
          try {
            const progressResponse = await fetch('http://localhost:5001/api/progress');
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 w-200 h-150 flex flex-col mx-auto">
      {/* Top accent bar */}
      <div className="h-1.5 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-t-xl flex-shrink-0"></div>
      
      {/* Device Header - Compact */}
      <div className="p-5 pb-3 flex-shrink-0">
        <div className="flex items-center space-x-4">
          <div className="w-6 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center flex-shrink-0">
            <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v14a2 2 0 002 2z" />
            </svg>
          </div>
          <div className="min-w-0 flex-1">
            <h3 className="text-lg font-bold text-gray-900 truncate">{device?.name || 'Device Name'}</h3>
            <p className="text-sm text-gray-600 truncate">{device?.type || 'IoT Sensor Device'}</p>
            <p className="text-xs text-gray-500 mt-1 line-clamp-2">
              {device?.description || 'Real-time environmental monitoring with automated alerts and cloud connectivity.'}
            </p>
          </div>
        </div>
      </div>
        ...
        {processing || showResults ? (
            ...
            {processing && (
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-700">Processing Drone Image</span>
              ...
              <div className="space-y-2">
                ...
                {processedImage && (
                  <div className="text-xs text-gray-600">
                    <p>✅ Image analyzed and annotated</p>
                    <p>📁 Results saved to: {processedImage.split('\\').pop()}</p>
                  </div>
                )}
                <div className="flex gap-2">
                  <button
                    onClick={() => processedImage && window.open(`file:///${processedImage}`, '_blank')}
                    className="flex-1 px-3 py-2 bg-green-600 text-white border border-green-600 rounded-lg hover:bg-green-700 hover:border-green-700 font-medium text-xs transition-colors"
                  >
                    View Results
                  </button>
                  <button
                    onClick={resetProcessing}
                    className="flex-1 px-3 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-50 border border-gray-200 hover:border-gray-300 rounded-lg font-medium text-xs transition-colors"
                  >
                    Reset
                  </button>
                </div>
          ...
          <div className="grid grid-cols-2 gap-3 h-full">
            ...
            <button
              ...
              className="flex flex-col items-center justify-center p-2 bg-orange-50 text-orange-700 border border-orange-200 rounded-lg hover:bg-orange-100 hover:border-orange-300 transition-colors"
            >
              <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              className="flex flex-col items-center justify-center p-2 bg-green-600 text-white border border-green-600 rounded-lg hover:bg-green-700 hover:border-green-700 transition-colors"
            >
              <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
      ...
      {/* Controls Section - Series Layout */}
        ...
        <div className="flex items-center justify-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              />
              <div className="w-8 h-4 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:bg-blue-600 transition-colors">
          ...
          <div className="flex items-center space-x-2">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                className="sr-only peer"
              />
              <div className="w-8 h-4 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:bg-green-600 transition-colors">
                <div className="absolute top-0.5 left-0.5 w-3 h-3 bg-white border border-gray-300 rounded-full transition-transform peer-checked:translate-x-4 peer-checked:border-white"></div>
              </div>
            </label>
            <span className={`text-xs font-semibold px-1.5 py-0.5 rounded ${csvEnabled ? 'text-green-700 bg-green-100' : 'text-gray-500 bg-gray-100'}`}>
              {csvEnabled ? 'ON' : 'OFF'}
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={() => onDetails && onDetails({
              ...device,
              fullDescription: device?.fullDescription || 'This IoT device provides comprehensive environmental monitoring capabilities including temperature, humidity, air quality, and motion detection. Features real-time data transmission, automated alerting system, cloud connectivity, and advanced analytics dashboard for predictive maintenance and operational insights.'
            })}
            className="flex-1 px-3 py-2 text-blue-600 hover:text-blue-800 hover:bg-blue-50 border border-blue-200 hover:border-blue-300 rounded-lg font-medium text-xs transition-colors group relative"
            title={`View detailed information about ${device?.name || 'this device'} including performance metrics, configuration settings, and operational history`}
          >
            ...
            <span className="text-[10px] text-gray-500 group-hover:text-blue-600 transition-colors block mt-0.5">
